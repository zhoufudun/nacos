...
public class NacosNamingService implements NamingService {
    ...
    public void registerInstance(String serviceName, String groupName, Instance instance) throws NacosException {
        ...
        if (instance.isEphemeral()) { // 零时节点，需要心跳维护
            BeatInfo beatInfo = beatReactor.buildBeatInfo(groupedServiceName, instance);
            beatReactor.addBeatInfo(groupedServiceName, beatInfo); // 启动定时心跳
        }
    ...
    public List<Instance> getAllInstances(String serviceName, String groupName, List<String> clusters,
        ...
        if (subscribe) {
            serviceInfo = hostReactor.getServiceInfo(NamingUtils.getGroupedName(serviceName, groupName), // 先走缓存，缓存没有的话在从远程查询
                    StringUtils.join(clusters, ","));
        } else {
            serviceInfo = hostReactor
                    .getServiceInfoDirectlyFromServer(NamingUtils.getGroupedName(serviceName, groupName), // 不走缓存，直接从远程查询
                            StringUtils.join(clusters, ","));
        }
        List<Instance> list;
