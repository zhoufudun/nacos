...
public class SecurityProxy {
    ...
    public boolean login(List<String> servers) {
        ...
        try {
            ...
            for (String server : servers) {
                if (login(server)) { // 向每个服务器发起登录请求
                    lastRefreshTime = System.currentTimeMillis(); // 记录上次登陆时间点
                    return true;
                }
            }
    ...
    public boolean login(String server) {
        ...
        if (StringUtils.isNotBlank(username)) {
            ...
            String url = "http://" + server + contextPath + LOGIN_URL; // http://127.0.0.1:8848/nacos/v1/auth/users/login
