...
public class NamingProxy {
    ...
    public static boolean syncData(byte[] data, String curServer) {
        ...
        headers.put(HttpHeaderConsts.CONNECTION, "Keep-Alive");
        headers.put(HttpHeaderConsts.CONTENT_ENCODING, "gzip"); // header={User-Agent=Nacos-Server:1.4.0, Accept-Encoding=gzip,deflate,sdch, Content-Encoding=gzip, Connection=Keep-Alive, Client-Version=1.4.0}
        ...
        try {
            RestResult<String> result = HttpClient.httpPutLarge(
                    "http://" + curServer + ApplicationUtils.getContextPath() + UtilsAndCommons.NACOS_NAMING_CONTEXT
                            + DATA_ON_SYNC_URL, headers, data); // url=
            if (result.ok()) {
                return true;
            }
